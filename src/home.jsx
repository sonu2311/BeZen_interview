          ...
          <div>
            <Box sx={{ ...style, width: 400 }}>
              <div className="card_title_div">
                <input className='card_title_input' placeholder="Title" value={title} onChange={(e)=>setTitle(e.target.value)}/>
              </div>
              <div>
                <textarea  rows="15" cols="20" className="message_text" type="text"
                  name="name" value={content} placeholder="Take a note" onChange={(e)=> setContent(e.target.value)}></textarea>
              </div> 
              ...
              <div className="hsplit" >
                  ...
                  <Box sx={{ '& button': { m: 1 } }}>
                    <div>
                      <Button size="small" onClick={() => cardDelete(carddetails.id)}>Delete</Button>
                    </div>
                  </Box> 
              </div>
            </Box>
...
export function Home(){
  ...
  const addNewNote = function(){
    const newItem = {title:"Untitled Note"+ counter,
    content: "",
    id:counter}
      ...
      <div style={{maxWidth: "1200px", margin: "auto", padding:"90px 1px"}} >
        ...
        <div className="hsplit">
          {cardList.slice(page*6-6, page*6).map((carddetails)=> (
            <Card key={carddetails.id} carddetails={carddetails} cardDelete={cardDelete} /> 
          ))}
        </div>
        <div className='hsplit'>
          <div className='col-lg-4 col-sm-6 col-xs-12'>
            <div className='add_button'>
              <Pagination count={Math.ceil(cardList.length/6)} variant="outlined" shape="rounded"   page={page} onChange={handleChange}/>
            </div>
          </div>
        </div> 
      </div>
